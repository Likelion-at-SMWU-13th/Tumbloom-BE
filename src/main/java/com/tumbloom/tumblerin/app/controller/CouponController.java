...
public class CouponController {
    ...
    @GetMapping("/coupons")
    public ResponseEntity<ApiResponseTemplate<List<AvailableCafeCouponDto>>> listAvailable(
        ...
        return ApiResponseTemplate.success(
                ...
                couponService.listAvailableCafes(cafeName)
        );
    }
    ...
    public ResponseEntity<ApiResponseTemplate<MyCouponDetailResponse>> issue(
            @Parameter(description = "쿠폰을 발급받을 카페 ID") @PathVariable Long cafeId,
            @AuthenticationPrincipal(expression = "user") User loginUser
    ) {
        return ApiResponseTemplate.success(
                SuccessCode.RESOURCE_RETRIEVED,   // 필요 시 RESOURCE_CREATED 등으로 교체
                couponService.issueCoupon(cafeId, loginUser)
        );
    }
    ...
    public ResponseEntity<ApiResponseTemplate<MyCouponListResponse>> myCoupons(
            @AuthenticationPrincipal(expression = "user") User loginUser
    ) {
        return ApiResponseTemplate.success(
                SuccessCode.RESOURCE_RETRIEVED,
                couponService.listMyCoupons(loginUser.getId())
        );
    }
    ...
    public ResponseEntity<ApiResponseTemplate<MyCouponDetailResponse>> myCouponDetail(
            @Parameter(description = "상세 조회할 쿠폰 ID") @PathVariable Long couponId,
            @AuthenticationPrincipal(expression = "user") User loginUser
    ) {
        return ApiResponseTemplate.success(
                SuccessCode.RESOURCE_RETRIEVED,
                couponService.getMyCoupon(loginUser.getId(), couponId)
        );
    }
    ...
    public ResponseEntity<ApiResponseTemplate<Void>> useMyCoupon(
