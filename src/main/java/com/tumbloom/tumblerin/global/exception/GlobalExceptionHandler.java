package com.tumbloom.tumblerin.global.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponseTemplate<String>> handleHttpMessageNotReadableException(HttpMessageNotReadableException e) {
        ...
        return ApiResponseTemplate.error(ErrorCode.INVALID_REQUEST, "잘못된 요청 형식입니다: " + e.getMessage());
    }
    ...
    public ResponseEntity<ApiResponseTemplate<String>> handleMissingParamException(MissingServletRequestParameterException e) {
        ...
        return ApiResponseTemplate.error(ErrorCode.INVALID_REQUEST, "요청 파라미터가 누락되었습니다: " + e.getParameterName());
    }
    ...
    private ErrorCode mapHttpStatusToErrorCode(HttpStatus status) {
        switch (status) {
            case BAD_REQUEST:
                return ErrorCode.INVALID_REQUEST;  // 400 Bad Request
            case UNAUTHORIZED:
            ...
            case INTERNAL_SERVER_ERROR:
                return ErrorCode.INTERNAL_SERVER_ERROR;  // 500 Internal Server Error
            case SERVICE_UNAVAILABLE:
                return ErrorCode.SERVICE_UNAVAILABLE;  // 503 Service Unavailable
            default:
                return ErrorCode.UNKNOWN_ERROR;  // 기타 모든 상태 코드는 UNKNOWN_ERROR로 처리
        }
    }
}